...
FactoryBot.define do
  factory 'params/logics/events/create', class: HashWithIndifferentAccess do
    event      { create('params/logics/events/create#event') }
    creator_id { create(:user).id }

    trait :invalid_finish do
      event { create('params/logics/events/create#event', :invalid_finish) }
    end

    trait :invalid do
      event { create('params/logics/events/create#event', :invalid) }
    end
  ...
  factory 'params/logics/events/create#event', class: Hash do
    title    { create(:string) }
    place    { create(:string) }
